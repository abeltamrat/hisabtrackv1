...
export default function LoansDebtsScreen() {
  ...
  const getProgress = (principal: number, remaining: number) => {
  ...
  const handleAddItem = async () => {
    ...
    await dispatch(addLoan({
      ...
      start_date: Date.now(),
      due_date: new Date(formData.dueDate).getTime() || Date.now(),
    ...
    // Find appropriate category for loans
  ...
  const handleEditItem = () => {
    ...
    dispatch(updateLoan({
      ...
      principal_amount: amount,
      interest_rate: parseFloat(formData.interestRate || '0'),
      due_date: new Date(formData.dueDate).getTime() || editingItem.due_date,
      status: remaining <= 0 ? 'PAID' : 'ACTIVE',
      remaining_balance: remaining,
    }));
    resetForm();
  ...
  const handleConfirmPayment = async () => {
    ...
    // Repayment of LENT (Given out) = INCOME (Money Back)
    // Repayment of DEBT (Taken) = EXPENSE (Money Out)

    // Find appropriate category for loan repayments
  ...
  const openEditModal = (item: Loan) => {
    setEditingItem(item);
    const paid = item.principal_amount - item.remaining_balance;
    setFormData({
  ...
  const onDateChange = (event: any, selectedDate?: Date) => {
    const currentDate = selectedDate || (formData.dueDate ? new Date(formData.dueDate) : new Date());
    if (Platform.OS === 'android') {
      setShowDatePicker(false);
    }
    setFormData({ ...formData, dueDate: currentDate.toISOString().split('T')[0] });
  };
        ...
        {!selectedGroup ? (
          ...
          <>
            {groupedItems[selectedGroup]?.items.map((item) => {
              const progress = getProgress(item.principal_amount, item.remaining_balance);
              const paidAmount = item.principal_amount - item.remaining_balance;
                ...
                >
                  ...
                  <View className="mb-4">
                    <View className="flex-row justify-between mb-2">
                      <Text className="text-slate-500 text-sm">{t('Payment Progress')}
                      </Text>
                    ...
                    {accounts.map(acc => (
                      ...
                      >
                        <Text className={`font-bold ${formData.accountId === acc.id ? 'text-emerald-700' : 'text-slate-600 dark:text-slate-400'
                          }`}>{acc.name} (${acc.balance.toFixed(0)})</Text>
                      </TouchableOpacity>
                    ))}
                  </ScrollView>
                </View>
